...
function getPayloadConfigFromPayload(
  ...
  // Check payload object first
  if (payload[key] && typeof payload[key] === "string") {
    const configKey = payload[key];
    return config[configKey];
  }

  // Then check payload.payload
  const innerPayload = payload.payload;
  if (innerPayload?.[key] && typeof innerPayload[key] === "string") {
    const configKey = innerPayload[key];
    return config[configKey];
  }

  // Finally try direct key
  return config[key];
}

const ChartTooltip = RechartsPrimitive.Tooltip;

// Fix: Define proper props interface for tooltip content
