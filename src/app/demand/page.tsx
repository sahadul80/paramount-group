...
import ConcernSection from "@/app/components/Demand/ConcernSection";
import ProductSection from "@/app/components/Demand/ProductSection";
import SelectedProductsList from "@/app/components/Demand/SelectedProductsList";
import SuccessModal from "@/app/components/Demand/SuccessModal";
import {
  Products,
  OrderEntry,
  DemandSummary,
...
export default function DemandPage() {
  ...
  useEffect(() => {
    const validateToken = async () => {
      ...
      if (!token || !user || !role || role !== "demand") {
        router.push("/login");
        return;
      }
  ...
  const handleProductSelect = (product: Products) => {
    ...
    if (existingIndex !== -1) {
      ...
      setOrderItems([
        ...orderItems,
        { product: product.product, quantity: 1, uom: product.uom },
      ]);
    }
  };
  ...
  return (
    <Card>
      <CardHeader className="flex items-center p-2 sm:p-4">
        <CardTitle className="text-lg font-bold">Production Demand</CardTitle>
      </CardHeader>
